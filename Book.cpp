...
class GT{
		...
		void create() 
    {
        ...
        getline(cin, root->label); // Read the entire line including spaces
        ...
        cin.ignore(); // Ignore any leftover newline characters
        getline(cin, root->child[i]->label); // Read the entire line including spaces
        ...
        for (int j = 0; j < root->child[i]->ch_count; j++) {
            ...
            cin.ignore(); // Ignore any leftover newline characters
            getline(cin, root->child[i]->child[j]->label); // Read the entire line including spaces
            ...
            for (int k = 0; k < root->child[i]->child[j]->ch_count; k++) {
                ...
                cin.ignore(); // Ignore any leftover newline characters
                getline(cin, root->child[i]->child[j]->child[k]->label); // Read the entire line including spaces
            }
        }
    }
}
